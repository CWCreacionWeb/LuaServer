          ...
          <?php else: foreach ($pgDbList as $db): ?>
            <div class="dbrow">
              ...
              <form method="post" enctype="multipart/form-data" class="dbimport row" style="gap:6px" onsubmit="return luaAskImportPg(event, this, '<?= e($db) ?>')">
                ...
                <input type="file" name="sqlfile" accept=".sql" id="pgSqlFile-<?= e($db) ?>" hidden onchange="document.getElementById('pgSqlName-<?= e($db) ?>').textContent = this.files[0] ? this.files[0].name : 'Ningún archivo'">
                <button type="button" class="btn ghost sm" onclick="document.getElementById('pgSqlFile-<?= e($db) ?>').click()">Elegir .sql</button>
                <span id="pgSqlName-<?= e($db) ?>" class="muted" style="font-size:11px">Ningún archivo</span>
        ...
        <?php else: foreach ($dbList as $db): ?>
          <div class="dbrow">
            ...
            <form method="post" enctype="multipart/form-data" class="dbimport row" style="gap:6px" onsubmit="return luaAskImportDb(event, this, '<?= e($db) ?>')">
              ...
              <input type="hidden" name="dbname" value="<?= e($db) ?>">
              <input type="file" name="sqlfile" accept=".sql" id="dbSqlFile-<?= e($db) ?>" hidden onchange="document.getElementById('dbSqlName-<?= e($db) ?>').textContent = this.files[0] ? this.files[0].name : 'Ningún archivo'">
              <button type="button" class="btn ghost sm" onclick="document.getElementById('dbSqlFile-<?= e($db) ?>').click()">Elegir .sql</button>
              <span id="dbSqlName-<?= e($db) ?>" class="muted" style="font-size:11px">Ningún archivo</span>
              <button class="btn ghost sm" type="submit">Importar</button>
            </form>
